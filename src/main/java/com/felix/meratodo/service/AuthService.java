...
//         return new TokenResponse(jwtService.generateAccessToken(newUser),jwtService.generateRefreshToken(newUser));
    ...
    public boolean isTeamOwner(Long teamId, Long userId) {
        return teamRepository.findById(teamId).map(t -> t.getOwner().getId().equals(userId)).orElse(false);
    }

    public boolean isTeamMember(Long teamId, Long userId) {
        return teamMembershipRepository.findByTeamIdAndUserId(teamId, userId).isPresent();
    }

    public boolean hasTeamRole(Long teamId, Long userId, TeamRole requiredRole) {
        return teamMembershipRepository.findByTeamIdAndUserId(teamId, userId)
                .map(m -> m.getTeamRole().ordinal() <= requiredRole.ordinal()) // Assume ordinal hierarchy: OWNER > LEADER > EDITOR > VIEWER
                .orElse(false);
    }

    public boolean isProjectOwner(Long projectId, Long userId) {
        return projectRepository.findById(projectId).map(p -> p.getOwner().getId().equals(userId)).orElse(false);
    }

    public boolean isProjectMember(Long projectId, Long userId) {
        Optional<Project> project = projectRepository.findById(projectId);
        if (project.isEmpty()) return false;
        if (project.get().getTeam().getId() != null) {
            return isTeamMember(project.get().getTeam().getId(), userId);
        }
        return isProjectOwner(projectId, userId); // Personal projects: only owner
    }

    public boolean hasProjectRole(Long projectId, Long userId, TeamRole requiredRole) {
        Optional<Project> project = projectRepository.findById(projectId);
        if (project.isEmpty()) return false;
        if (project.get().getTeam().getId() != null) {
            return hasTeamRole(project.get().getTeam().getId(), userId, requiredRole);
        }
        return isProjectOwner(projectId, userId); // Owners have full access
    }

    public boolean isTaskAssignee(Long taskId, Long userId) {
        return taskAssignmentRepository.existsByTask_IdAndUser_Id(taskId, userId);
    }

    public boolean canAccessTask(Long taskId, Long userId, boolean forEdit) {
        Optional<Task> task = taskRepository.findById(taskId);
        if (task.isEmpty()) return false;
        Long projectId = task.get().getProject().getId();
        if (forEdit) {
            return hasProjectRole(projectId, userId, TeamRole.EDITOR); // EDITOR+ for edits
        }
        return isProjectMember(projectId, userId) || isTaskAssignee(taskId, userId);
    }
}
